  ...
  return (
    <>
      {filteredGamesList.length <= 0 ? (
        ...
        filteredGamesList.slice(0, 10).map((deal, index) => (
          <li
            className="deals__list"
            key={index}
            onClick={() => fetchGameDataByGameID(`${deal.gameID}`)}
          >
            <Link to={`/selected-game/${deal.gameID}`} className="deals__link">
              ...
              <div className="deals__price">
                <span className="deals__discount">{`-${Math.round(
                  Number(deal.savings)
                )}%`}</span>
                <span className="deals__sale-price">
                  {`${deal.salePrice}$ `}
                  <span className="deals__normal-price">{`${deal.normalPrice}$`}</span>
                </span>
              </div>
            </Link>
          </li>
